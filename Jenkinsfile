pipeline {
    ...
    environment {
        ...
        REMOTE_HOST = "3.36.114.146"    // 원격(spring) 서버 IP(Public IP)

        REMOTE_DIR = "/home/ec2-user/deploy"    // 원격 서버에 파일 복사할 경로
        SSH_CREDENTIALS_ID = "4fcbcdd5-8412-4cb6-9d54-d998d5b8a135" // Jenkins SSH 크레덴셜 ID 복사
    }
    ...
    stages {
        ...
        stage('Maven Build') {
            steps {
                ...
                // sh 'echo Hello' : 리눅스 명령어 실행, sh는 linux shell 명령어를 실행하다는 뜻
            }
        }
